...
function mkConsent(o: Partial<ConsentRecord> = {}): ConsentRecord {
  return {
    ...
    approved_paths: ["src/"], approved_tools: ["repo.search", "repo.fetch", "repo.tree", "repo.symbols"],
...
await describe("Grants", async () => {
  await it("create with consent", () => {
    ...
    const g = createGrant({ user_id: U, client_id: C, repo_id: R, snapshot_id: S, allowed_tools: ["repo.search"], allowed_paths: ["src/"], data_budget: defaultGrantBudget(), expiry: new Date(Date.now() + 36e5).toISOString(), revoked_at: null }, c);
  ...
  await it("re-consent for path expansion", () => {
    ...
    const g = createGrant({ user_id: U, client_id: C, repo_id: R, snapshot_id: S, allowed_tools: ["repo.search"], allowed_paths: ["src/ui/"], data_budget: defaultGrantBudget(), expiry: new Date(Date.now() + 36e5).toISOString(), revoked_at: null }, c);
    const grant = getGrant((g as { grant_id: string }).grant_id)!;
    assert.equal(requiresReConsent(grant, "repo.search", "src/utils/x.ts").required, true);
  });
});
...
await describe("Policy Engine", async () => {
  ...
  beforeEach(() => {
    ...
    const c = mkConsent(); recordConsent(c);
    token = mkToken().token;
    const g = createGrant({ user_id: U, client_id: C, repo_id: R, snapshot_id: S, allowed_tools: ["repo.search","repo.fetch","repo.tree","repo.symbols"], allowed_paths: ["src/"], data_budget: defaultGrantBudget(), expiry: new Date(Date.now()+36e5).toISOString(), revoked_at: null }, c);
    gid = (g as { grant_id: string }).grant_id;
  });

  await it("authorize valid call", () => {
    assert.equal(authorizeToolCall(token, gid, "repo.search", R, S, "src/a.ts").allowed, true);
  });
  await it("deny wrong repo", () => {
    assert.equal(authorizeToolCall(token, gid, "repo.search", "bad", S, "src/a.ts").allowed, false);
  });
  await it("deny wrong snapshot", () => {
    assert.equal(authorizeToolCall(token, gid, "repo.search", R, "bad", "src/a.ts").allowed, false);
  });
  await it("deny revoked grant", () => {
    revokeGrant(gid);
    assert.equal(authorizeToolCall(token, gid, "repo.search", R, S, "src/a.ts").allowed, false);
  });
  await it("deny unapproved tool", () => {
    const c = mkConsent({ approved_tools: ["repo.search"] }); recordConsent(c);
    const g2 = createGrant({ user_id: U, client_id: C, repo_id: R, snapshot_id: S, allowed_tools: ["repo.search"], allowed_paths: ["src/"], data_budget: defaultGrantBudget(), expiry: new Date(Date.now()+36e5).toISOString(), revoked_at: null }, c);
    const ngid = (g2 as { grant_id: string }).grant_id;
    assert.equal(authorizeToolCall(token, ngid, "repo.fetch", R, S, "src/a.ts").allowed, false);
  });
  await it("deny path outside allowed", () => {
    assert.equal(authorizeToolCall(token, gid, "repo.search", R, S, "tests/x.ts").allowed, false);
  });
  await it("deny wrong user", () => {
    const t2 = mkToken({ user_id: "evil" });
    assert.equal(authorizeToolCall(t2.token, gid, "repo.search", R, S, "src/a.ts").allowed, false);
  });
  await it("authDeniedResponse returns structured error", () => {
    const r = authorizeToolCall("bad", gid, "repo.search", R, S, "src/a.ts");
