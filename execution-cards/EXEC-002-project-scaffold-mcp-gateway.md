# EXEC-002：项目骨架与 MCP 网关

状态：ready_for_ai_execution（可由 AI 执行）。
依赖：EXEC-001 complete（完成）。

## 1. Objective（目标）

在默认执行根目录创建最小 `Node.js`（节点运行时） + `TypeScript`（类型脚本）项目骨架，建立只读 `MCP gateway`（模型上下文协议网关）和工具注册壳，不实现仓库读取。

## 2. Scope（范围）

Allowed files（允许修改）：
- `implementation/package.json`
- `implementation/tsconfig.json`
- `implementation/src/server.ts`
- `implementation/src/tools/registry.ts`
- `implementation/src/config.ts`
- `implementation/src/errors.ts`
- `implementation/tests/server-smoke.test.ts`
- `implementation/docs/mcp-gateway-contract.md`

Forbidden changes（禁止）：
- 不实现文件读取。
- 不运行用户仓库命令。
- 不添加第三方 MCP 框架作为主边界；核心使用官方 `@modelcontextprotocol/sdk`。
- 不引入 UI widget（界面小组件）。

## 必交产物（Required Deliverables）

Primary deliverables（主产物；路径相对本项目根目录）：
- implementation/package.json
- implementation/tsconfig.json
- implementation/src/server.ts
- implementation/src/tools/registry.ts
- implementation/src/config.ts
- implementation/src/errors.ts
- implementation/tests/server-smoke.test.ts
- implementation/docs/mcp-gateway-contract.md

Closeout deliverables（收口产物；终止状态回写前必须存在）：
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/cycle1.input.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/cycle1.receipt.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/closure.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/validator_result.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/handshake.json（仅整卡委派给子代理执行时必须先创建）

Artifact existence validation（产物存在性校验）：
- 终止状态回写前，执行器必须用 `Test-Path` 或等效方法确认每个必交产物存在。
- required artifact path missing（必交产物路径缺失）、路径仍为通配符且未列出实际文件、或产物无法从证据追踪时，不得写 `complete`（完成）。
## 3. Required Work（必须执行）

- 初始化 package（包）和 TypeScript（类型脚本）配置。
- 注册四个工具名称：`repo.search`、`repo.fetch`、`repo.tree`、`repo.symbols`。
- 每个工具必须有 title（标题）、description（说明）、inputSchema（输入模式）、outputSchema（输出模式）、annotations（注解）。
- 所有工具实现先返回 `not_implemented` 结构化错误，不读取任何文件。
- 提供统一错误对象和 `structuredContent`（结构化内容）。
- 服务初始化 instructions（指引）必须声明仓库内容不可信、禁止整仓导出、工具只读。

## 4. Acceptance Criteria（验收标准）

- AC-001：`npm test` 或等效测试能验证工具注册表包含四个工具。
- AC-002：所有工具注解为 `readOnlyHint=true`、`destructiveHint=false`、`openWorldHint=false`。
- AC-003：调用任一工具不会读取文件，只返回 `not_implemented`。
- AC-004：依赖清单不包含 `fastmcp`、`mcp-framework` 等第三方主框架。

## 5. Validation（校验）

Commands（命令）：
- `npm install`
- `npm test`
- `npm run typecheck`
- `npm run lint`（若配置）

## 6. Required Evidence（所需证据）

- `package.json` 依赖清单。
- 工具注册测试输出。
- typecheck（类型检查）输出。

## 7. Completion Writeback（完成回写）

changed files:
- implementation/package.json
- implementation/tsconfig.json
- implementation/src/server.ts
- implementation/src/tools/registry.ts
- implementation/src/config.ts
- implementation/src/errors.ts
- implementation/tests/server-smoke.test.ts
- implementation/docs/mcp-gateway-contract.md

created artifacts:
- list actual created files
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/cycle1.input.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/cycle1.receipt.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/closure.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/validator_result.json
- execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/handshake.json（仅整卡委派给子代理时）

validation commands:
- list exact commands

validation results, including exit code:
- include output summary and exit code

skipped validations and reason:
- list skipped checks

protected files unchanged:
- confirm no repo source read and no UI widget introduced

remaining blockers:
- list blockers or none

completion status:
- complete | blocked | failed

## 8. Non-Completion Rule（非完成规则）

通用规则：required artifact path missing（必交产物路径缺失）、终止状态缺少 Claude closeout（Claude 收口复核）侧车证据、`validate_closure.py` 未通过、或 Claude 候选问题未分类时，不得写 `complete`（完成）。

如果任何工具能访问文件系统，或依赖不是官方 MCP SDK 主边界，不得写 `complete`（完成）。
## Claude Closeout Review（终止回写前外部复核）

- 在写入 `complete`、`blocked`、`failed` 或 `aborted` 等终止状态前，必须调用 `claude-taskcard-review`（Claude 任务卡复核）并把输入、回执、`closure`（收口）和 `validator_result`（校验结果）写入同名 `.claude-review` 侧车目录。
- `closure.json` 必须通过 `$CODEX_HOME/skills/claude-taskcard-review/scripts/validate_closure.py`；若 `CODEX_HOME` 不可用，必须记录解析到的脚本路径或阻塞原因。
- `Claude findings`（Claude 候选问题）只作为 `advisory`（建议性）证据；执行器必须把每个候选问题分类为 `accepted_blocking`、`accepted_non_blocking`、`rejected_false_positive` 或 `rejected_out_of_scope`，并写明本地证据。
- `complete`（完成）要求最新 Claude receipt（回执）为 `call_status=ok`、所有 `accepted_blocking` 已解决、`validate_closure.py` 报告可写完成；否则只能按本地证据写 `blocked`、`failed` 或 `aborted`。
- 若整卡委派给子代理执行，子代理在业务工作前必须先写 `execution-cards/EXEC-002-project-scaffold-mcp-gateway.claude-review/handshake.json`，且其中 `agent_role` 必须是 `executor`（执行者）。
